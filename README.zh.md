...
1. **提示词注入层**：每个 agent 创建时（`agent/created`）向 system prompt 追加 5 个方法论节（紧随官方 persona 之后），共 32 条可执行准则。每条准则为「触发条件 + 做法 + 禁忌 + 原文出处」格式，模型每轮生成前都会读到。
...
3. **设置层**：浏览器设置页「毛泽东思想方法论」卡片 —— 总开关 / 精简模式 / 五节独立开关（经插件自身路由读写，保存后新会话生效）。

## 安装

### 方式一：插件市场（推荐）

1. 打开 DSH 的插件市场（zat-dsh-engine），搜索 **dsh-mao-method**（GitHub `dsh-plugin` 主题）
2. 点安装（`pnpm add github:cstyx1990-design/dsh-mao-method`），重启 DSH
3. 重启后进入设置页 →「毛泽东思想方法论」，按需开关

### 方式二：命令行

```bash
dsh plugin --profile <你的profile名> add github:cstyx1990-design/dsh-mao-method
```

### 方式三：超级模组注入器（本机开发）

```bash
bash scripts/fetch-corpus.sh   # 拉取全文语料（可选但推荐）
